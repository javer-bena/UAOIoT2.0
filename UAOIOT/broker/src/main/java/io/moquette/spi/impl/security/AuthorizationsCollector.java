...
public class AuthorizationsCollector implements IAuthorizator {
    ...
    public void clearUserAuthorizations() {
        m_userAuthorizations.clear();
    }

    public void addUser(String username) {
        if (!m_userAuthorizations.containsKey(m_currentUser)) {
            m_userAuthorizations.put(username, new ArrayList<Authorization>());
        }
    }

    public void addAuthorization(String username, String permissionTemp, String topicTemp) throws ParseException{
        List<Authorization> userAuths = m_userAuthorizations.get(m_currentUser);
        try {
            Authorization.Permission permission = Authorization.Permission.valueOf(permissionTemp);
            Topic topic = new Topic(topicTemp);
            userAuths.add(new Authorization(topic, permission));
